...
export default function EditarRolPage() {
  ...
  // /roles/[id] → tomamos el último segmento
  ...
  useEffect(() => {
    async function init() {
      ...
      if (!esAdmin) {
        setError("No tiene permisos para editar roles.");
        setLoading(false);
        return;
      }
      ...
      try {
        const rol = await apiGet(`/api/roles/${id}/`);
  ...
  async function handleSubmit(e) {
    ...
    try {
      setGuardando(true);
      await apiPatch(`/api/roles/${id}/`, {
        nombre: nombre.trim(),
        slug: slug.trim(),
        descripcion: descripcion.trim() || "",
      });

      setMensajeOk("Rol actualizado correctamente.");
      setTimeout(() => {
        router.push("/roles");
      }, 800);
    } catch (err) {
      ...
      <div className="p-6 max-w-xl mx-auto space-y-4">
        <div>
          ...
          <p className="text-xs text-slate-500">
            Modifique el nombre, slug o descripción del rol.
          </p>
        </div>
