...
export default function EditarEventoPage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      if (!puedeGestionarEventos(user)) {
        setError("No tiene permisos para editar eventos.");
        setLoading(false);
        return;
      }

      if (!id) {
        setError("No se pudo determinar el evento.");
        setLoading(false);
        return;
      }

      try {
        const ev = await apiGet(`/api/eventos/${id}/`);

        const nombreInicial = ev.nombre || ev.titulo || "";
        const descInicial = ev.descripcion || ev.detalle || "";
        const fechaInicial =
          ev.fecha ||
          ev.fecha_inicio ||
          ev.fecha_evento ||
          "";
        const estadoInicial = ev.estado || ev.status || "";

        setNombre(nombreInicial);
        setDescripcion(descInicial);
        setFecha(fechaInicial ? fechaInicial.substring(0, 10) : "");
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      // ⚠️ Ajusta los nombres de campos según tu serializer de Evento
      const payload = {
        nombre: nombre.trim(),
      ...
      if (fecha) {
        payload.fecha = fecha; // o fecha_inicio / fecha_evento según tu backend
      } else {
        payload.fecha = null;
      }
      ...
      await apiPatch(`/api/eventos/${id}/`, payload);

      setMensajeOk("Evento actualizado correctamente.");
      setTimeout(() => {
        router.push("/eventos");
      }, 700);
    } catch (err) {
      ...
      <div className="p-6 max-w-xl mx-auto space-y-4">
        <div>
          ...
          <p className="text-xs text-slate-500">
            Actualice los datos del evento seleccionado.
          </p>
        </div>
          ...
          >
            ...
            <div className="space-y-1">
              ...
              <input
                ...
                placeholder="Planificado, En curso, Finalizado, etc."
                className="w-full border border-slate-200 rounded-md px-3 py-1.5 text-xs outline-none focus:ring-1 focus:ring-brand-secondary focus:border-brand-secondary bg-white"
              />
            </div>
