...
export default function EditarDocumentoROIPage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      if (!puedeGestionarROI(user)) {
        setError("No tiene permisos para editar documentos ROI.");
        setLoading(false);
        return;
      }

      if (!id) {
        setError("No se pudo determinar el documento.");
        setLoading(false);
        return;
      }

      try {
        const [doc, eventosData, empleadosData] = await Promise.all([
          apiGet(`/api/documentos-roi/${id}/`),
          apiGet("/api/eventos/"),
        ...
        setFechaEvento(doc.fecha_evento || "");
        setFechaLimiteOferta(doc.fecha_limite_oferta || "");
        setEstadoSeguimiento(doc.estado_seguimiento || "No se conoce");
        setEstadoProceso(doc.estado_proceso || "Pendiente");
        setOrigenClasificacion(doc.origen_clasificacion || "Manual");
        setMotivoSuspendido(doc.motivo_suspendido || "");
        setEnlaceDocumento(doc.enlace_documento || "");
        setEventoId(doc.evento || doc.evento_id || "");
        setCreadoPorId(doc.creado_por || doc.creado_por_id || "");
      } catch (err) {
        console.error(err);
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const payload = {
        ...
        archivo: null, // adaptar a FormData si usas subida de archivo real
        enlace_documento: enlaceDocumento.trim() || null,
        evento_id: eventoId || null,
        creado_por_id: creadoPorId || null,
      };

      await apiPatch(`/api/documentos-roi/${id}/`, payload);

      setMensajeOk("Documento ROI actualizado correctamente.");
      setTimeout(() => {
  ...
  return (
    <MainLayout currentUser={currentUser}>
      <div className="p-6 max-w-3xl mx-auto space-y-4">
        ...
        {/* El formulario es idéntico al de "nuevo", solo que con valores cargados */}
        <form
