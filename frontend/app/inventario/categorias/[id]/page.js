...
export default function EditarCategoriaPage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      if (!puedeGestionarInventario(user)) {
        setError("No tiene permisos para editar categorías.");
        setLoading(false);
        return;
      }
      ...
      try {
        const cat = await apiGet(`/api/categorias/${id}/`);
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      await apiPatch(`/api/categorias/${id}/`, {
        nombre: nombre.trim(),
      ...
      <div className="p-6 max-w-xl mx-auto space-y-4">
        <div>
          ...
          <p className="text-xs text-slate-500">
            Actualice los datos de la categoría seleccionada.
          </p>
        </div>
